...
const Home: React.FC = () => {
  return (
    <main className="flex flex-col items-center w-full">
              ...
              <div className="relative z-10 w-full aspect-square bg-gradient-to-br from-white to-slate-50 rounded-3xl border border-white/40 shadow-[0_32px_64px_-16px_rgba(0,0,0,0.1)] overflow-hidden group flex items-center justify-center">
                ...
                <div className="relative z-20 w-full h-full p-12 flex items-center justify-center">
                  <img 
                    ...
                    className="max-w-full max-h-full object-contain drop-shadow-[0_20px_40px_rgba(0,128,128,0.2)] animate-float transition-all duration-700 group-hover:scale-110"
                    onError={(e) => {
                      (e.target as HTMLImageElement).src = 'https://images.unsplash.com/photo-1546776310-eef45dd6d63c?auto=format&fit=crop&q=80&w=800';
                    }}
                  />
                </div>
              </div>
      ...
      {/* Overview Section */}
      <section className="w-full bg-white border-y border-black/5 py-16">
      ...
      <section className="w-full px-4 md:px-10 py-28 max-w-[1280px]">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
          <div className="relative rounded-[2rem] overflow-hidden aspect-[4/3] group shadow-2xl">
            <img 
              src={ASSETS.home.aboutSection} 
              className="w-full h-full object-cover transition-transform duration-1000 group-hover:scale-110" 
              alt="GEOOSE in action" 
              onError={(e) => { (e.target as HTMLImageElement).src = 'https://images.unsplash.com/photo-1504384308090-c894fdcc538d?auto=format&fit=crop&q=80&w=800' }}
            />
          </div>
            ...
            {[
              ...
              <div key={idx} className="bg-white/5 backdrop-blur-sm rounded-[2rem] overflow-hidden border border-white/10 group hover:border-secondary/50 transition-all duration-500">
                <div className="h-64 relative overflow-hidden">
                  <img src={item.img} alt={item.title} className="w-full h-full object-cover grayscale opacity-50 group-hover:grayscale-0 group-hover:opacity-100 transition-all duration-700" onError={(e) => { (e.target as HTMLImageElement).src = 'https://images.unsplash.com/photo-1518152006812-edab29b069ac?auto=format&fit=crop&q=80&w=800' }} />
